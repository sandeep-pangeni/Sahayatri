<?php
$new_boookings = $obj->Query("SELECT * from bookings where user = $uid and status = '2'");

$cancelled_bookings_with_d = $obj->Query("SELECT * from bookings where driver!='' and user = $uid and status =  '2' ORDER BY time desc");

$cancelled_bookings_without = $obj->Query("SELECT * from bookings where  driver='0' and user = $uid and status =  '2' ORDER BY time desc");
...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'cancel') {
        ...
        echo "<script> window.location.href='" . base_url('cancelled-bookings.php') . "'</script>";
    }
}
        ...
        <div class="col-md-12">
            ...
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                    <?php if ($cancelled_bookings_with_d) { ?>
                        <table class="table dataTable table-responsive-lite">
                            ...
                            <tbody>
                                <?php foreach ($cancelled_bookings_with_d as $key => $value) : ?>
                                    <tr>
                                        <td><?= ++$key ?></td>
                                        <td>
                                            <?php
                                                    ...
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <img src="<?= file_driver_url($driver_p) ?>" class="img-fluid w-100">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div><?php } ?>
                                        ...
                                        <td>
                                            <a href="<?= base_url('cancelled-bookings.php?action=d&id=' . $value->id) ?>" class="btn btn-light border border-danger text-danger btn-sm" onclick="return confirm('Are you sure you want to delete?')"> <i class="fa fa-trash"></i> Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                    ...
                    <?php } else { ?>
                        <div class="text-centser">
                            <h5 class="text-danger">No cancelled bookings with driver !</h5>
                        </div>
                    <?php } ?>
                </div>

                <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                    <?php if ($cancelled_bookings_without) { ?>
                        <table class="table dataTable table-responsive-lite">
                            <thead>
                                <tr>
                                    <th>S.N</th>
                            ...
                            <tbody>
                                <?php foreach ($cancelled_bookings_without as $key => $value) : ?>
                                    <tr>
                                        <td><?= ++$key ?></td>
                                        <td>
                                            <?php
                    ...
                    <?php } else { ?>
                        <div class="text-centser">
                            <h5 class="text-danger">No cancelled bookings without driver!</h5>
                        </div>
                    <?php } ?>
                </div>
            </div>
