<?php
$new_boookings = $obj->Query("SELECT * from bookings where user = $uid and status = '1'");


$confirmed_bookings_with_d = $obj->Query("SELECT * from bookings where driver!='' and user = $uid and status =  '1' ORDER BY time desc");

$confirmed_bookings_without = $obj->Query("SELECT * from bookings where  driver='0' and user = $uid and status =  '1' ORDER BY time desc");
...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'd') {
        ...
        echo "<script> window.location.href='" . base_url('confirmed-bookings.php') . "'</script>";
    }
}
?>
                            ...
                            <tbody>
                                <?php foreach ($confirmed_bookings_with_d as $key => $value) : ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="<?= base_url('confirmed-bookings.php?action=d&id=' . $value->id) ?>" class="btn btn-light border border-danger text-danger btn-sm" onclick="return confirm('Are you sure you want to delete?')"> <i class="fa fa-trash"></i> Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
