<?php
// $new_bookings = $obj->Query("SELECT * from bookings where status =  '0' ORDER BY time desc");

$new_bookings_with_d = $obj->Query("SELECT * from bookings where driver!='0' and status =  '0' ORDER BY time desc");

$new_bookings_without = $obj->Query("SELECT * from bookings where  driver='0' and status =  '0' ORDER BY time desc");
...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'cancel') {
        ...
        echo '<script>alert("Boooking cancelled!")</script>';
                ...
                <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                    <?php if ($new_bookings_with_d) { ?>
                        <table class="table DataTable table-responsive-lite" id="exData">
                            ...
                            <tbody>
                                <?php foreach ($new_bookings_with_d as $key => $value) : ?>
                                    <tr>
                                        <td><?= ++$key ?></td>
                                        <td>
                                            <?php
                    ...
                    <?php } else { ?>
                        <div class="text-centser">
                            <h5 class="text-danger">No pending bookings !</h5>
                        </div>
                    <?php } ?>
                </div>

                <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                    <?php if ($new_bookings_without) { ?>
                        <table class="table DataTable table-responsive-lite" id="exData">
                            <thead>
                                <tr class="bg-white">
                                    <th>S.N</th>
                            ...
                            <tbody>
                                <?php foreach ($new_bookings_without as $key => $value) : ?>
                                    <tr>
                                        <td><?= ++$key ?></td>
                                        <td>
                                            <?php
                                        ...
                                        <td class="text-nowrap">
                                            <a href="<?= base_url('new-bookings.php?action=cancel&id=' . $value->id) ?>" class="btn btn-outline-danger  btn-sm" onclick="return confirm('Are you sure you want to cancel?')"> <i class="bi bi-trash-fill"></i> Cancel </a>

                                            <a href="<?= base_url('new-bookings.php?action=confirm&id=' . $value->id) ?>" class="btn btn-success btn-sm" onclick="return confirm('Are you sure you want to Confirm?')"> <i class="bi bi-trash-fill"></i> Confirm </a>
